...
img_mask = cv2.inRange(img_hsv, lower_blue, upper_blue)    #바이너리이미지 휙득, 범위내의 이미지 흰색 나머지 검은색

img_result = cv2.bitwise_and(img_color, img_color, mask = img_mask)    #bitwise_and(InputArray src1, InputArray src2, OutputArray dst, InputArray mask=noArray())
